...
# II --- Load data
df = pd.read_parquet('boombustpo_estimates_vintages_twosided_pfonly.parquet')
...
def plot_linechart(data, cols, nice_names, colours, dash_styles, y_axis_title, main_title, output_suffix):
    ...
    fig.write_image('Output/BoomBustPO_Vintage_TwoSided_PFOnly_' + output_suffix + '.png', height=768, width=1366)
    fig.write_html('Output/BoomBustPO_Vintage_TwoSided_PFOnly_' + output_suffix + '.html')
    return fig
...
fig_vintages = plot_linechart(
    ...
    main_title='Vintages of Current Output Gap Estimates (PF Only (Two-Sided))',
    output_suffix='OutputGap'
)
telsendimg(conf=tel_config,
           path='Output/BoomBustPO_Vintage_TwoSided_PFOnly_OutputGap.png',
           cap='Vintages of Current Output Gap Estimates (PF Only (Two-Sided))')
...
fig_rev = plot_areachart(
    ...
    main_title='Revisions in Current Output Gap Across Consecutive Vintages (PF Only (Two-Sided))',
    show_legend=False,
    ymin=-5,
    ymax=5,
...
telsendimg(conf=tel_config,
           path='Output/BoomBustPO_Vintage_TwoSided_PFOnly_OutputGap_Revisions.png',
           cap='Revisions in Current Output Gap Across Consecutive Vintages (PF Only (Two-Sided))')


# IV --- Notify
telsendmsg(conf=tel_config,
           msg='boombustpo_plot_vintages_twosided_pfonly: COMPLETED')
