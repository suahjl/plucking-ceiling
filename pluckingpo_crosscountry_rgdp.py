...
df_raw = df_raw[~((df_raw['country'] == 'Bulgaria') & (df_raw['quarter'] < '2000Q1'))]
...
def compute_pluck(data, entities_label, rows_per_epi):
    ...
    for entity in tqdm(list_entities):
        ...
        stdev = np.std(d['rgdpsa_qoq'])
        factor1 = 0.8  # adjust to include / exclude obvious episodes

        # if qoqsa < 0, met with qoqsa >= 0 in next 4Q, abs(qoqsa) >= stdev, AND NO TROUGHS IN PAST 4Q, then a trough
        d.loc[
            (
                    (d['rgdpsa_qoq'] < 0) &
                    (d['rgdpsa_qoq'].shift(-1) >= 0) &
                    (d['rgdpsa_qoq'].shift(-2) >= 0) &
                    (d['rgdpsa_qoq'].shift(-3) >= 0) &
                    (d['rgdpsa_qoq'].shift(-4) >= 0)
                    &
                    (np.abs(d['rgdpsa_qoq']) >= factor1 * stdev)
            ),
            'trough'
        ] = 1
        d.loc[
        ...
        if d['epi'].max() == 0:
            ...
            d_expcon['subsequent_contraction_pace'] = d_expcon['subsequent_contraction_pace'] * (-1)
...
df, df_expcon, df_conexp = compute_pluck(data=df_raw, entities_label='country', rows_per_epi=None)  # 24
