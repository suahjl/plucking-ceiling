...
df_kf_ind_full = pd.read_parquet('boombustpo_input_data_kf.parquet')  # twosided
...
for t_end in tqdm(list_t_ends):
    ...
    # Merging
    if round == 1:
        df_final = pd.DataFrame(df['output_gap_kf']).rename(columns={'output_gap_kf': t_end})  # kf
    elif round > 1:
...
df_final.to_parquet('boombustpo_estimates_vintages_twosided_kfonly.parquet', compression='brotli')

# VI --- Notify
telsendmsg(conf=tel_config,
           msg='boombustpo_compute_vintages_twosided_kfonly: COMPLETED')
