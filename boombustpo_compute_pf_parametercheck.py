...
for hplambda in tqdm(list_hplambdas):
    for alpha in list_alphas:
        ...
        # apply filter to raw y, k, n using specified lambda
        for i, j in zip(list_col_ln, list_col_ln_trend):
            cycle, trend = sm.filters.hpfilter(
                df_sub.loc[~df_sub[i].isna(), i], lamb=hplambda)
            df_sub[j] = trend  # don't replace original with trend component
...
df_po_consol.to_parquet('boombustpo_estimates_pf_po_parametercheck' +
...
df_og_consol.to_parquet('boombustpo_estimates_pf_og_parametercheck' +
                        file_suffix_fcast + '.parquet', compression='brotli')
...
fig_og = plot_linechart(
    ...
    y_axis_title="%",
    main_title="Sensitivity check for two-sided HP-filtered PF output gap estimates",
    use_forecast_choice=use_forecast
)
file_name = path_output + "BoombustPO_PF_OG_ParameterCheck"
...
fig_po = plot_linechart(
    ...
    nice_names=list_nice_param_combos,
    y_axis_title="%",
    main_title="Sensitivity check for two-sided HP-filtered PF potential output estimates",
    use_forecast_choice=use_forecast
)
file_name = path_output + "BoombustPO_PF_PO_ParameterCheck"
...
telsendmsg(conf=tel_config,
           msg='boombustpo_compute_pf_parametercheck: COMPLETED')
