...
# II --- Load data
df_hd = pd.read_parquet('boombustpo_estimates_pf_onesided_hd' + file_suffix_fcast + '.parquet')  # onesided
...
def plot_histdecomp(input, use_forecast_choice):
    ...
    fig_ceiling.update_layout(title='Historical Decomposition of Potential Output YoY Growth (One-Sided)',
                              yaxis_title='Percentage Point, %YoY growth',
                              barmode='relative',
                              plot_bgcolor='white',
                              hovermode='x',
                              font=dict(size=20, color='black'))
    fig_ceiling.write_html('Output/BoomBustPO_OneSided_HistDecomp_PO' + file_suffix_fcast + '.html')
    fig_ceiling.write_image('Output/BoomBustPO_OneSided_HistDecomp_PO' + file_suffix_fcast + '.png', height=768, width=1366)
    ...
    fig_ob.update_layout(title='Historical Decomposition of Observed Output YoY Growth (One-Sided)',
                         yaxis_title='Percentage Point, %YoY growth',
                         barmode='relative',
                         plot_bgcolor='white',
                         hovermode='x',
                         font=dict(size=20, color='black'))
    fig_ob.write_html('Output/BoomBustPO_OneSided_HistDecomp_Obs' + file_suffix_fcast + '.html')
    fig_ob.write_image('Output/BoomBustPO_OneSided_HistDecomp_Obs' + file_suffix_fcast + '.png', height=768, width=1366)

    return fig_ceiling, fig_ob


fig_ceiling, fig_ob = plot_histdecomp(input=df_hd, use_forecast_choice=use_forecast)
for i in ['Output/BoomBustPO_OneSided_HistDecomp_PO' + file_suffix_fcast + '.png',
          'Output/BoomBustPO_OneSided_HistDecomp_Obs' + file_suffix_fcast + '.png']:
    telsendimg(conf=tel_config,
               path=i)

# IV --- Notify
telsendmsg(conf=tel_config,
           msg='boombustpo_plot_histdecomp_onesided: COMPLETED')
