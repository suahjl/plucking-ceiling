...
def compute_ceilings(data, levels_labels, ref_level_label, bounds_timing_shift):
        ...
        if round == 1:
            ...
            for diff, levels, trough, peak, epi, cepi, pace, ceiling, cpace \
                    ...
                    tqdm(zip(col_diff, col_levels,
                             ...
                             col_ceiling,
                             col_cpace)):
                ptref_ceiling = ceiling
                ceiling_plusb = ceiling + '_ub'
                ceiling_minusb = ceiling + '_lb'
                d_consol[ceiling_plusb] = d_consol[ptref_ceiling] + \
                                          np.abs(d_consol[ptref_ceiling] - d_consol[ceiling_minusb])  # same indices
...
df_check = df[['ln_gdp15_ceiling', 'ln_gdp15_ceiling_lb', 'ln_gdp15_ceiling_ub']]
...
def prodfunc(
    ...
    d['gdp15_ceiling_ub'] = np.exp(d['ln_gdp15_ceiling_ub'])
    ...
    list_col_keep = ['output_gap', 'output_gap_lb', 'output_gap_ub',
                     'gdp15_ceiling', 'gdp15_ceiling_lb', 'gdp15_ceiling_ub',
                     'gdp15',
                     'capital_ceiling',
                     'labour_ceiling',
                     'tfp_ceiling',
...
fig_gdp = plot_linechart(data=df_pd,
                         cols=['gdp15', 'gdp15_ceiling', 'gdp15_ceiling_lb', 'gdp15_ceiling_ub'],
                         colours=['crimson', 'black', 'lightgrey', 'lightgrey'],
                         y_axis_title='ppt',
...
fig_og = plot_linechart(data=df_og,
                        ...
                        colours=['darkblue', 'lightgrey', 'lightgrey', 'lightcoral', 'black'],
                         y_axis_title='%',
