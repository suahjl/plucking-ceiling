...
for t_end in tqdm(list_t_ends):
    ...
    # Merging
    if round == 1:
        df_final = pd.DataFrame(df['output_gap_pf']).rename(columns={'output_gap_pf': t_end})  # pf
    elif round > 1:
...
df_final.to_parquet('boombustpo_estimates_vintages_twosided_pfonly.parquet', compression='brotli')

# VI --- Notify
telsendmsg(conf=tel_config,
           msg='boombustpo_compute_vintages_twosided_pfonly: COMPLETED')
