...
df_bb = pd.DataFrame(df_bb['output_gap_pf']).rename(columns={'output_gap_pf': 'output_gap'})
...
def scatterplots(data_full,
    ...
    for x_col, y_col, x_nice_name, y_nice_name, colour_core, colour_outlier, marker_size, line_width, main_title, output_suffix \
        ...
        fig.update_layout(title=main_title,
                          ...
                          showlegend=False)
        fig.write_image('Output/BoomBustPO_TwoSidedPF_MacroComparison_' + output_suffix + '.png',
                        height=768, width=1366)
        fig.write_html('Output/BoomBustPO_TwoSidedPF_MacroComparison_' + output_suffix + '.html')
...
for i, j in tqdm(zip(list_suffixes, list_main_titles)):
    telsendimg(conf=tel_config,
               path='Output/BoomBustPO_TwoSidedPF_MacroComparison_' + i + '.png',
               cap=j)
...
telsendmsg(conf=tel_config,
           msg='boombustpo_plot_macro_comparison_twosidedpf: COMPLETED')
